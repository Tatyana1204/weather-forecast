    ...
    return (
        <div>
            <div className={s.detailsBox}>
                <QualityBox2 title='Качество воздуха' value={detailsWeather.air_quality} image={airQuality}/>
                <QualityBox1 title={'Закат'} value={detailsWeather.sunset} image={sunset}
                             condition={'Восход в ' + detailsWeather.sunrise + '.'}/>
                <QualityBox1 title={'Ощущается как'} value={detailsWeather.feels_like} image={temperature}
                             condition='Примерно как фактически.'/>
                <QualityBox1 title={'Ветер'} value={detailsWeather.wind} image={wind}
                             condition={'Направление: ' + detailsWeather.windDegree + '.'}/>
                <QualityBox1 title={'Влажность'} value={detailsWeather.humidity} image={humidity}
                             condition={'Точка росы сейчас: ' + detailsWeather.dewPoint + '.'}/>
                <QualityBox1 title={'Видимость'} value={detailsWeather.visibility} image={visibility}
                             condition='Хорошая видимость.'/>
                <QualityBox1WithRange title={'УФ-Индекс'} value={detailsWeather.uv} image={sun} max={12}/>
                <QualityBox1 title={'Осадки'} value={detailsWeather.precipitationAmount} image={rain}
                             condition={'Завтра ожидается ' + detailsWeather.precipitationAmount_tm}/>
                <QualityBox1 title={'Давление'} value={detailsWeather.pressure} image={pressure}
